...
App 优先保存 `b64_json`，也兼容内嵌 data URL、网关常见的 `image_base64` / `partial_image_b64` / `result.data` 包装和普通 HTTPS 图片链接。若服务商返回异步图片任务，App 会在同一 API 地址轮询任务，不会重新提交生成请求。不会为 GPT Image 默认添加官方不支持的 `response_format` 参数。

如果只有部分用户出现“接口返回成功，但没有找到图片数据”，通常不是手机分辨率问题，而是这些用户命中的上游账号返回了空的 200 响应、异步任务或不可达的图片外链。应用会把这几种情况区分开：空响应提示检查上游返回结构；已有外链但下载失败则只显示“重新下载”，不会再次扣生图费用。请在 sub2api 的每一个可能被调度的生图账号上都打开 URL 转 base64，并检查上游日志是否存在空输出或 502 后被错误包装成 200。
