...
一个简洁的 Android AI 图片创作与多模态对话客户端，可连接兼容 OpenAI Images、Chat Completions、Responses 或 Claude Messages 的服务商。生图和对话可分别配置独立 API 地址、密钥与模型，不要求共用。应用不内置密钥或固定模型；历史与文件保存在手机本地，执行请求时必要内容会发给用户选择的服务商。
...
5. 图片可直接用于视觉理解；PDF 在 Android 手机本地逐页转换成图片，再由视觉模型理解，每份最多 12 页。超过页数、加密、损坏的 PDF 会在请求前报错，不会偷偷截取前几页。文件辅助生图会提取为提示词，不会保证完全复刻页面布局。
...
限制：每次最多 4 张图片、4 份文档；单个附件最多 20MB，图片与文档合计最多 30MB。对话携带最近 12 轮成功消息（含图片与文件），历史附件合计超过 30MB 时会在请求前提示，请新建会话或减少附件。最终编码后请求体还受 32MB 限制，Base64 会增加体积，因此可能需要进一步缩小附件。文本上下文也有大小保护。DOCX／PPTX 等请先转 PDF。聊天文本可长按选择或一键复制。
