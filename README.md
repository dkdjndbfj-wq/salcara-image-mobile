...
- ChatGPT 风格的统一对话与会话管理；默认自动判断每条消息是问答、文件/图片分析还是图片创作，不强制切换页面
- 保存并切换多个服务商；对话 API 与图片 API 可完全分开，图片服务商可以只配置 Images 权限
- 独立的应用设置、服务商管理、网络诊断和更新入口
- 自动读取 `/v1/models`，分开配置生图和对话模型，也可手动填写模型 ID
- 保留“仅对话 / 仅图片创作”手动覆盖；默认模式下图片只是示意时仍走对话 API，明确要求生成/修改时才进入图片流程
- 生图与对话可使用不同服务商／密钥，在同一会话中讨论生成结果，再继续创作
- 文件辅助生图：先由所选多模态对话模型分析/确认作图要求，再发起生图；两次调用分别计费。没有对话服务商时，自动模式不会直接发起付费生图
...
1. 默认进入「自动判断」，输入文字，或用加号添加图片／文件。
2. 对话模型必须支持所用输入类型。出现在 `/models` 中不代表支持视觉或 PDF；不支持时请自行改选模型，不会自动换接口产生重复计费。
3. 如果生图分组只支持图片，另建一个对话服务商，填写它的地址、密钥、对话模型。自动模式会在需要时使用这个独立对话服务商；会话不会丢失，必要历史会发送到新选服务商。
4. 自动模式下，上传图片后说“分析/识别/描述”只会走对话 API；说“参考这张图生成海报”才会先由对话 API 整理要求，再调用图片编辑 API。上传 PDF 等文件后先分析，下一条明确要求生成时会沿用当前会话的文件上下文。
